...
export async function ocrImage(imageBase64: string): Promise<OcrResult> {
  ...
  const prompt = `你是一个负责初中数学错题录入的识别助手。请分析这张试卷/练习照片，照片中可能同时包含多道题目、学生作答、批改痕迹。
...
6. 给出整张图片的 summary。
7. 每道题补充 question_region，表示该题在整张图片中的大致区域。
...
10. 如果图片里明显有 2 题或 3 题，必须分开列出，不能合并成一道题。
11. 只要存在不确定，就用保守、谨慎的表述，不要写“全部正确”“没有错题”之类过度自信的总结。
12. question_region 要尽量紧贴当前题目，不要跨到下一题；如果题目下方紧跟下一题题号，question_region 应该在下一题题号之前结束。
13. 如果一道题有选择项，请把 question_region 包含题干和选项，但不要把下一题题干带进去。
14. 如果某道题的图形单独成块，question_payload.diagram.region 只标图形，不要用它替代整道题的 question_region。

区域坐标规则：
- 使用整张图片的相对坐标。
- 统一返回 0 到 1000 的整数 left、top、width、height。
- 题目区域要尽量覆盖完整题干和作答，图形区域要尽量覆盖完整图形，不要裁掉边缘。
